...
class AdminMissionPageBean extends AdminPageBean {
  ...
  public function getAddEditPage($Mission, $title, $postAction) {
    ...
    $args = array(
      ...
      '#',
      // Url pour Annuler - 3
      $this->getQueryArg(array(self::CST_ONGLET=>CST_MISSION)),
      // Id de la Mission - 4
      $Mission->getId(),
      // Type de l'action - 5
      $postAction,
  ...
  public function getListingPage() {
    ...
    $WpPostsPublished = $this->WpPostServices->getArticles(__FILE__, __LINE__, array(self::CST_POSTSTATUS=>CST_PUBLISH, CST_ORDERBY=>$orderby, CST_ORDER=>$order));
    $WpPostsFuture = $this->WpPostServices->getArticles(__FILE__, __LINE__, array(self::CST_POSTSTATUS=>CST_FUTURE, CST_ORDERBY=>$orderby, CST_ORDER=>$order));
    switch ($post_status ) {
    ...
    $queryArg = array(self::CST_ONGLET=>CST_MISSION, CST_ORDERBY=>$orderby, CST_ORDER=>$order);
    // Subs
    ...
    $args = array(
      ...
      $this->getQueryArg(array(self::CST_ONGLET=>CST_MISSION, CST_POSTACTION=>'add')),
      // subs - 4
      $subs,
      // pagination - 5
      $strPagination,
